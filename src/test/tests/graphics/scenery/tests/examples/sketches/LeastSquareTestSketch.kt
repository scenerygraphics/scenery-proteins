...
class LeastSquareTestSketch {
    ...
    fun testLeastSquare() {
        ...
        assertEquals(leastSquare(points), Vector3f(0f, 0f, 1f))
    }

    private fun leastSquare (points: List<Vector3f>): Vector3f {
        val centroid = getCentroid(points)
        val transPoints = ArrayList<Pair<Vector3f, Vector3f>>(points.size)
        ...
        return Vector3f(sumXLength / abs, sumYLength / abs, sumZLength / abs).normalize()
    }
